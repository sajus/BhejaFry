...
exports.delRecruiterById = function(req, res) {
	if (req.session.roles === 'Administrator') {
		...
		var sql_isEmpidDeleted = "SELECT a.empid FROM recruiter_tbl a WHERE a.empid = " + sqlString.escape(empid) + " AND a.recycleBin = 0 LIMIT 1";
		sequelize.query(sql_isEmpidDeleted).success(function(rows) {
			if (rows.length === 0) {
				// The recruiter email does not exist.
				res.status(500).send("The recruiter you specified is incorrect.");
			} else {
				// The recruiter email does exist.
				var sql_updateRecruiter = "UPDATE recruiter_tbl a SET a.recycleBin = 1 WHERE empid = " + sqlString.escape(rows[0].empid) + " AND a.recycleBin = 0";
				sequelize.query(sql_updateRecruiter).success(function() {
					res.send(req.params);
				}).error(function(error) {
					console.log('SQL Error:\n');
					console.log(error);
	...
	if (req.session.roles === 'Administrator') {
		...
		sequelize.query(sql_isEmpidDeleted).success(function(rows) {
			if (rows.length === 0) {
				...
				sql_updateRecruiterDetails += "fistname = " + sqlString.escape(firstname) + ", ";
